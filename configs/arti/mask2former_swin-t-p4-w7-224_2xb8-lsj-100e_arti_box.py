...
train_dataloader = dict(
    ...
    dataset=dict(
        data_root=data_root,
        metainfo=metainfo,
        ann_file='train/annotation_coco_box.json',
        data_prefix=dict(img='train/')))
val_dataloader = dict(
    dataset=dict(
        data_root=data_root,
        metainfo=metainfo,
        ann_file='val/annotation_coco_box.json',
        data_prefix=dict(img='val/')))
test_dataloader = dict(
    dataset=dict(
        data_root=data_root,
        metainfo=metainfo,
        ann_file='test/annotation_coco_box.json',
        data_prefix=dict(img='test/')))

# Modify metric related settings
val_evaluator = dict(_delete_=True, type='CocoMetric',  metric=['bbox', 'segm'], ann_file=data_root + 'val/annotation_coco_box.json')
test_evaluator = dict(_delete_=True, type='CocoMetric',  metric=['bbox', 'segm'], ann_file=data_root + 'test/annotation_coco_box.json')
